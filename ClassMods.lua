...
function ClassMods.CheckIfKnown(spell, item)
	...
	elseif (item) and (C_Item.GetItemInfo(item) ) then -- Items are easier to check, single call, no helpers
		return true
	end
	return nil
end
...
function ClassMods.NameToSpellID(spellName)
	local spellLink
	spellLink = C_Spell.GetSpellLink(spellName)
	if spellLink then
...
function ClassMods.NameToItemID(itemName)
	local itemName
	itemLink = C_Spell.GetSpellLink(itemName)
	if itemLink then
...
function ClassMods.GetSpellCost(id, spec)
	...
	if (select(2, UnitClass("player")) == "WARRIOR") then
		...
		elseif (id == 204488) and AuraUtil.FindAuraByName(C_Spell.GetSpellName(202573), "player") then -- Vengence: Focused Rage buff
			returnCost = spellCost - ceil(spellCost * .67)
		elseif (id == 190456) and AuraUtil.FindAuraByName(C_Spell.GetSpellName(202574), "player") then -- Vengence: Ignore Pain buff
			returnCost = spellCost - ceil(spellCost * .67)
		end
	end
...
function ClassMods.Timer_Create(self)
	local scaler = CreateFrame('Frame', nil, self, "BackdropTemplate")
	scaler:SetAllPoints(self)
	local timer = CreateFrame('Frame', nil, scaler, "BackdropTemplate")
	timer:Hide()
...
function ClassMods.CheckForDebuff(spell, target, owner)
	local buffIndex = ClassMods.getAuraIndex(target, tonumber(spell) and C_Spell.GetSpellName(tonumber(spell) ), (owner == "PLAYERS") and "PLAYER|HARMFUL" or "HARMFUL")
	if not buffIndex then return end
	local aura = C_UnitAuras.GetAuraDataByIndex(target, buffIndex,  (owner == "PLAYERS") and "PLAYER|HARMFUL" or "HARMFUL")
	...
	if (target == "target") and strSpellId and ClassMods.spellTracker.spells[strSpellId] then
		if (UnitGUID("target") and (UnitGUID("target") == ClassMods.spellTracker.spells[strSpellId][1]) ) or (ClassMods.spellTracker.spells[strSpellId][1] == UnitGUID("player") ) then
			if (ClassMods.spellTracker.spells[strSpellId][2] > GetTime() ) then
				if (not name) then
					name, icon = C_Spell.GetSpellName(spell), C_Spell.GetSpellTexture(spell)
					count = 1
				end
...
function ClassMods.CheckForBuff(spell, target, owner)
	local buffIndex = ClassMods.getAuraIndex(target, tonumber(spell) and C_Spell.GetSpellName(tonumber(spell) ) or spell, (owner == "PLAYERS") and "PLAYER|HELPFUL" or "HELPFUL")
	if not buffIndex then return end
	local aura = C_UnitAuras.GetAuraDataByIndex(target, buffIndex, (owner == "PLAYERS") and "PLAYER|HELPFUL" or "HELPFUL")
	local name = aura.name
	local icon= aura.icon
	local count = aura.applications
	local duration = aura.duration
...
function ClassMods.GetTimerInfo(spell, item, target, timerType, owner, internalcd, lastTime)
	...
	-- ITEM COOLDOWN
	if item then
		itemName, itemLink, _, _, _, _, _, _, _, itemTexture = C_Item.GetItemInfo(tonumber(item) or item)
		if (not itemLink) then
			return (nil), (0), (0), (0)
		end
		startTime, duration, enable = GetItemCooldown(select(5, ClassMods.ParseItemLink(itemLink) ) ) -- Why the hell doesn't C_Item.GetItemInfo return the ID
	...
	if (timerType == "COOLDOWN") then
		...
		local spellCooldown = C_Spell.GetSpellCooldown(name)
		startTime = spellCooldown.startTime
		duration = spellCooldown.duration
		enable = spellCooldown.isEnabled
		local chargeInfo = C_Spell.GetSpellCharges(name)
	...
	if (timerType == "ICOOLDOWN") then
		...
		if icon then
			...
			name, icon2 = C_Spell.GetSpellName(spell), C_Spell.GetSpellTexture(spell)
...
function ClassMods.GetTimerIconTexture(spell, item)
	if item then -- ITEM
		local itemName, _, _, _, _, _, _, _, _, itemTexture = C_Item.GetItemInfo(tonumber(item) or item)
		if itemName and itemTexture then
			return itemTexture
		end
	else -- SPELL
		local name, icon = C_Spell.GetSpellName(spell), C_Spell.GetSpellTexture(spell)
...
function ClassMods.MakeSmooth(powerFrame)
	...
	powerFrame.smoother = powerFrame.smoother or CreateFrame("Frame", nil, powerFrame, "BackdropTemplate")
...
function ClassMods.MakeFrame(recycle, ...)
	local frame = recycle or CreateFrame(..., "BackdropTemplate")
...
function ClassMods.getAuraIndex(unitName, spellName, filter)
	for i = 1, 99 do
		local aura = C_UnitAuras.GetAuraDataByIndex(unitName, i, filter)
		if (aura) then
