...
local function ClassMods_PriorityLineConstructor()
	local frame = CreateFrame("Frame", "ClassMods_PriorityLine" .. AceGUI:GetNextWidgetNum("ClassMods_PriorityLine"), UIParent, "BackdropTemplate")
	frame:Hide()

	local btnup = CreateFrame("Button", nil, frame, "BackdropTemplate")
	btnup:SetWidth(24)
	btnup:SetHeight(24)
	...
	local btndn = CreateFrame("Button", nil, frame, "BackdropTemplate")
	btndn:SetWidth(24)
	btndn:SetHeight(24)
	...
	local frameicon = CreateFrame("Frame", nil, frame, "BackdropTemplate")
...
local function SortImportList(listTable)
	table.sort(listTable, function(a,b)
		local aComp = (a[1] ~= nil) and (tonumber(a[1]) and (C_Spell.GetSpellName(a[1]) ) or a[1]) or (tonumber(a[2]) and (select(1, C_Item.GetItemInfo(a[2]) ) ) or a[2])
		local bComp = (b[1] ~= nil) and (tonumber(b[1]) and (C_Spell.GetSpellName(b[1]) ) or b[1]) or (tonumber(b[2]) and (select(1, C_Item.GetItemInfo(b[2]) ) ) or b[2])
...
local function DoPanel3(num)
	...
	for i=1,#DB.timers["timerbar"..num].timers do
		...
		if (DB.timers["timerbar"..num].timers[i][1]) then
			name, icon = C_Spell.GetSpellName(DB.timers["timerbar"..num].timers[i][1]), C_Spell.GetSpellTexture(DB.timers["timerbar"..num].timers[i][1])
		else
			name, _, _, _, _, _, _, _, _, icon--[[texture]] = C_Item.GetItemInfo(DB.timers["timerbar"..num].timers[i][2])
		end
...
local function DoPanel2(num)
	...
	local function DrawGroup(container, group)
		...
		for i=1,#ClassMods.timerbarDefaults[drawTab] do
			...
			local name = C_Spell.GetSpellName(ClassMods.timerbarDefaults[drawTab][i][1] or ClassMods.timerbarDefaults[drawTab][i][2])
			cb:SetLabel(name)
