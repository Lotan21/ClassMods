...
function ClassMods.SetupAnnouncements()
	...
	ClassMods.F.Announcements = ClassMods.F.Announcements or CreateFrame("Frame", "CLASSMODS_ANNOUNCEMENTS", UIParent, "BackdropTemplate") -- Handler frame, nothing more.
			...
			elseif (self._subEvent == "SPELL_AURA_REMOVED") and (self._sourceGUID == UnitGUID("player") and (self._destGUID == UnitGUID("player"))) then
				for key,val in pairs(ClassMods.db.profile.announcements.announcements) do
					if (ClassMods.db.profile.announcements.announcements[key].enabled and ClassMods.db.profile.announcements.announcements[key].announceend) then
						if ((self._spellId == ClassMods.db.profile.announcements.announcements[key].spellid) or (self._spellName == C_Spell.GetSpellName(ClassMods.db.profile.announcements.announcements[key].spellid)) or (self._spellName == ClassMods.db.profile.announcements.announcements[key].spellid)) and (ClassMods.GetChatChan(ClassMods.db.profile.announcements.announcements[key][strlower(ClassMods.GetGroupType() ).."chan"]) ~= "NONE") then
