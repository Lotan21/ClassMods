...
function ClassMods.CanUseClicktocast(spell)
	...
	local usable, _ = C_Spell.IsSpellUsable(spell)
	if (not usable) then
...
function ClassMods.SetupClickToCast()
	...
	if (tonumber(ClassMods.db.profile.clicktocast.spell)) then
		spellToCast = C_Spell.GetSpellName(tonumber(ClassMods.db.profile.clicktocast.spell))
	else
	...
	ClassMods.F.ClickToCast = ClassMods.F.ClickToCast or CreateFrame("Frame", "CLASSMODS_CLICKTOCAST", UIParent, "BackdropTemplate") -- Handler frame, nothing more.
