...
local mod, strupper, select, pairs, unpack, tonumber, floor, UnitAura, UnitCastingInfo = mod, strupper, select, pairs, unpack, tonumber, floor, C_UnitAuras.GetAuraDataByIndex, UnitCastingInfo
...
local function checkTargetForIndicator(caller, target)
	if(caller.indicatorTrigger == _Debuff) then
		caller.___name, caller.___texture, caller.___stacks, caller.____, caller.___duration, caller.___expires = UnitAura(target, caller.checkID and C_Spell.GetSpellName(caller.checkID) or caller.checkName, nil, "PLAYER|HARMFUL")
	elseif(caller.indicatorTrigger == _Buff) then
		caller.___name, caller.___texture, caller.___stacks, caller.____, caller.___duration, caller.___expires = UnitAura(target, caller.checkID and C_Spell.GetSpellName(caller.checkID) or caller.checkName, nil, "PLAYER|HELPFUL")
	end
...
function ClassMods.SetupIndicators()
	...
	for key,val in pairs(ClassMods.db.profile.indicators.indicators) do
		if ClassMods.db.profile.indicators.indicators[key] and ClassMods.db.profile.indicators.indicators[key].enabled then
			...
			ClassMods.F.Indicators[index].value:SetJustifyV("MIDDLE")
				...
				function(self, elapsed)
					...
					if (self.active == true) then -- This alert is currently active, check if it's still valid
						...
						if ((self._name) and (not self.missing)) or ((not self._name) and (self.missing)) then -- Now it's active!
							if ((InCombatLockdown()) and (self.combat)) or (not self.combat) then
								self.active = true
								self.Icon:SetTexture(self._texture or C_Spell.GetSpellTexture(self.checkID) or C_Spell.GetSpellName(ClassMods.NameToSpellID(self.checkName)))
								self.Icon:Show()
								if self._stacks and (self._stacks > 1) then
									self.stacks:SetText(self._stacks)
									self.stacks:Show()
