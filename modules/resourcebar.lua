...
local select, UnitAura = select, C_UnitAuras.GetAuraDataByIndex
local _
	...
	if (ClassMods.enableticks == true) and (ClassMods.db.profile.resourcebar.ticks[playerSpec][1][1] == true) then
		...
		for i=1,5 do
			if (ClassMods.db.profile.resourcebar.ticks[playerSpec][i][1] == true) and (ClassMods.db.profile.resourcebar.ticks[playerSpec][i][4] == true) then
				name1 = C_Spell.GetSpellName(ClassMods.db.profile.resourcebar.ticks[playerSpec][i][2])
...
function ClassMods.GetBaseGeneration()
	...
	if ClassMods.powerGenerationSpells[playerSpec] then
		for i=1,#ClassMods.powerGenerationSpells[playerSpec] do
			if (C_Spell.GetSpellName(tonumber(ClassMods.powerGenerationSpells[playerSpec][i][1])) == castingSpell) then
...
function ClassMods.SetupResourceBar()
	...
	if ClassMods.db.profile.resourcebar.enabled then
		...
		ClassMods.F.ResourceBar:SetIgnoreParentAlpha(true)
		...
		if ClassMods.db.profile.resourcebar.enableprediction then
			ClassMods.F.ResourceBar.PredictionFrame = ClassMods.F.ResourceBar.PredictionFrame or CreateFrame("StatusBar", nil, ClassMods.F.ResourceBar, "BackdropTemplate")
		...
		if ClassMods.db.profile.resourcebar.autoattackbar then
			...
			ClassMods.F.ResourceBar.autoAttackFrame = ClassMods.F.ResourceBar.autoAttackFrame or CreateFrame("StatusBar", nil, ClassMods.F.ResourceBar, "BackdropTemplate")
		...
		if (ClassMods.enableticks == true) then
			...
			for ii=1,5 do
				if (ClassMods.db.profile.resourcebar.ticks[playerSpec][ii][1] == true) then
					name = C_Spell.GetSpellName(ClassMods.db.profile.resourcebar.ticks[playerSpec][ii][2])
					...
					if (name and cost and (cost > 0) and (ClassMods.db.profile.resourcebar.ticks[playerSpec][ii][6] == powerType)) then
						...
						if (ClassMods.db.profile.resourcebar.ticks[playerSpec][ii][7] == true) then
							icon = C_Spell.GetSpellTexture(ClassMods.db.profile.resourcebar.ticks[playerSpec][ii][2])
						else
				...
				if (ClassMods.db.profile.resourcebar.ticks[playerSpec][1][1] == true) then
					...
					for i=1,5 do
						if (ClassMods.db.profile.resourcebar.ticks[playerSpec][i][1] == true) and (self["indicatorTick"..i] ~= nil) and (ClassMods.db.profile.resourcebar.ticks[playerSpec][1][1] == true) then
							self.name2 = C_Spell.GetSpellName(ClassMods.db.profile.resourcebar.ticks[playerSpec][i][2])
				...
				if (select(2, UnitClass("player")) == "ROGUE") and (playerSpec == 2) then
					checkStacksFunction = function(self)
						...
						for i=1,#ClassMods.rollTheBones do
							local buffIndex = ClassMods.getAuraIndex("player", C_Spell.GetSpellName(ClassMods.rollTheBones[i]), "HELPFUL")
					...
					local buffIndex = ClassMods.getAuraIndex("player", C_Spell.GetSpellName(ClassMods.db.profile.resourcebar.stacks[playerSpec][i][2]), "HELPFUL")
					if buffIndex then
	...
	for i=1,numBars do
		ClassMods.F.StackBars[i] = ClassMods.F.StackBars[i] or CreateFrame("Frame", nil, ClassMods.F.StackBarsHost, "BackdropTemplate")
